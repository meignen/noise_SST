function [STFT,SST1,SST2,SST3,SST4,omega,omega2,omega2n_1,omega2n_2,omega3,omega3n_1] = sstn_simple(s,gamma,sigma,ft,bt)
% sstn : computes the STFT of a signal and different versions of synchrosqueezing/reassignment.
%   Uses a Gaussian window, the filter has the same length as the signal. 
%
% INPUTS:  
...
%   omega2: instantaneous frequency (SST2) 
%   omega2n_1: first order approximation of omega2
%   omega2n_2: second order approximation of omega2 
%   omega3: instantaneous frequency (SST3)
%   omega3n_1: first order approximation of omega3
...
omega2n_1 = zeros(neta,nb);
omega2n_2 = zeros(neta,nb);
omega3 = zeros(neta,nb);
omega3n_1 = zeros(neta,nb);
...
for b=1:nb
    ...
    omega2n_2(:,b)=(ft-1)'+imag(vg(:,2)/2/pi./vg(:,3))+imag((vg(:,2)).^3/2/pi./(vg(:,1).*(vg(:,3)).^2));
    omega2n_1(:,b)=(ft-1)'+imag(vg(:,2)/2/pi./vg(:,3));
    %SST3
    ...
    %% Aproximation of omega3
    det_u3=-3*vg(:,1).*vg(:,2).*vg(:,3)+vg(:,1).^2.*vg(:,4)+2*vg(:,2).^3;
    det_D=vg(:,1).*(vg(:,3).*vg(:,5)-vg(:,4).^2)-vg(:,2).*(vg(:,2).*vg(:,5)-vg(:,3).*vg(:,4))+vg(:,3).*(vg(:,2).*vg(:,4)-vg(:,3).^2);
    term1=-vg(:,3)./vg(:,1);
    omega3n_1(:,b)=omega2(:,b)+(1/2/pi).*imag(det_u3./det_D).*real(term1);
