...
sigma = 0.03;
ft =1:N/2;bt=1:N;
[STFT,FSST,FSST2] = sstn_simple(s,gamma,sigma,ft,bt);
...
figure
plot((Loc-1)/N,Cs(1,Loc)-1,'o',(Loc1-1)/N,Cs(1,Loc1)-1,'s',(Loc2-1)/N,Cs0(1,Loc2)-1,'*',...
     tk,260*ones(1,length(tk)),'d',(0:N-1)/N,Cs(1,:)-1,(0:N-1)/N,Cs1(1,:)-1,'--',(0:N-1)/N,Cs0(1,:)-1,'-.','Linewidth',2,'MarkerSize',15)
ax = gca;
ax.FontSize = 30;
xlim([xn xN])
ylim([255 263]);
...
%% signal
s = exp(2*pi*1i*(phi1))+2*exp(2*pi*1i*(phi2));
% the points bar t_k
tk = (2*(0:floor((4*(260-230)-1)/2))+1)/(4*(260-230));
gamma=0; sigma = 0.03;
